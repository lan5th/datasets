  ...
  def _get_page_content(self, pipeline, file_paths, dl_manager):
    ...
    def download_wet_file(path, dl_dir):
      ...
      if tf.io.gfile.exists(out_path):
        ...
        with tfds.core.utils.incomplete_dir(out_path) as tmp_dir:
          tfds.download.downloader.get_downloader().download(url, tmp_dir)
          tf.io.gfile.rename(
              os.path.join(tmp_dir, os.path.basename(out_path)),
              out_path,
              overwrite=True)
